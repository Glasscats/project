...
# 建立上层框架
frameUp = tkinter.Frame(app)
frameUp.pack()
# 上层框架第一个按钮
butOne = tkinter.Label(frameUp, text="选择画笔宽度")
butOne.pack(side=tkinter.LEFT, padx=5, pady=5)
sizevar = tkinter.IntVar()
SizeList = [x for x in range(1, 10, 1)]
sizevar.set(SizeList[0])
ShowOption = tkinter.OptionMenu(frameUp, sizevar, *SizeList)
ShowOption.pack(side=tkinter.LEFT)
# 上层框架第二个按钮
butTwo = tkinter.Button(frameUp, text="颜色选择", fg="black")
butTwo.pack(side=tkinter.LEFT, padx=5, pady=5)
# 上层框架第三个按钮
butThree = tkinter.Button(frameUp, text="字体选择", fg="black")
...
def getter(widget):
    ...
    ImageGrab.grab().crop((x,y,x1,y1)).save("./img.jpg",'jpeg')
